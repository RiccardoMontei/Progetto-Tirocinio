...
public class RewardController : MonoBehaviour {
	...
	public void OnCollisionEnter(Collision other){
		if (trainer.activeTraining && trainer.zombieBrainTrainer) { //Se sto addestrando Zombie Brain) 
	...
	public void OnTriggerEnter(Collider other){
		if (trainer.activeTraining && trainer.noZombieBrainTrainer) { //Se sto addestrando No Zombie Brain) 

			if (other.gameObject.CompareTag ("block")) { // se il cubo entra nel trigger di un'altro cubo
				timer = 0; //azzero il mio timer
				agent.SetReward (0.1f); //Assegno un reward basso per incentivare la ricerca
				agent.Done (); 
			}
		}
		if (trainer.activeTraining && trainer.zombieBrainChestTrainer) { //Se sto addestrando Zombie Brain Chest) 

			if (other.gameObject.CompareTag ("chest")) {// se il cubo entra nel trigger della chest
				timer = 0;
		...
		if (trainer.activeTraining && trainer.noZombieBrainTrainer) { //Se sto addestrando no Zombie Brain) 
			...
			if (other.gameObject.CompareTag ("block")) { 
				...
				if (timer > 5f) { //Se rimango nel trigger per 5 secondi
					...
					agent.SetReward (2f); //Assegno una ricompensa sostanziosa per incentivare la vicinanza con i cubi
					agent.Done ();
					timer = 0;
				}
			}
