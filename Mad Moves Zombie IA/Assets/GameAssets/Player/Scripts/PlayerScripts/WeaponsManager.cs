...
public class WeaponsManager : MonoBehaviour {
	...
	void Update () {
		isCrafting = GameObject.FindGameObjectWithTag ("CraftingSystem").GetComponent<CraftingSystem> ().activeCrafting;
	...
	private void ReloadFunction(){
		...
		if (Input.GetKey("r") && 
			...
			!animator.GetBool("IsReloading") && !isCrafting) { // e non sto gia ricaricando
	...
	private void ChangeWeapon(){
		if (Input.GetButtonDown ("ChangeWeapon") && !animator.GetBool("IsReloading") && !animator.GetBool("IsShotting")  && !isCrafting) {// Se premo ALT e non sto ricaricando o sparando
			changeWeapon=true;
	...
	private void Shot(){
		...
		} else if (!activeWeapon  && !isCrafting) { //Con l'arma secondaria
